<template>
	<div class="navbar-bookmarks flex items-center">
		...
		<div class="bookmark-container">
			<feather-icon icon="StarIcon" :svgClasses="['stoke-current text-primary', textColor]" class="cursor-pointer p-2" @click.stop="showBookmarkPagesDropdown = !showBookmarkPagesDropdown" />
			<div v-click-outside="outside" class="absolute bookmark-list w-1/3 xl:w-1/4 mt-4" v-if="showBookmarkPagesDropdown">
				...
				:searchLimit="5"
				placeholder="Componentes Favoritos..."
...
export default {
	...
	computed: {
		...
		starredPagesLimited: {
			get() {
				return this.starredPages.slice(0, 15)
			},
			set(list) {
		...
		starredPagesMore: {
			get() {
				return this.starredPages.slice(15)
			},
			set(list) {
		...
		textColor() {
			return {'text-white': this.$store.state.mainLayoutType === 'vertical' && this.navbarColor != (this.$store.state.theme === 'dark' ? "#191919" : "#fff") }
		}
	},
	methods: {
		selected(obj) {
