...
export default {
	...
	watch: {
		...
		isThemeDark(val) {
			const color = this.navbarColor == "#fff" && val ? "#191919" : "#fff"
	...
	methods: {
		...
		updateNavbar(val) {
		if (val == "static") this.updateNavbarColor(this.isThemeDark ? "#191919" : "#fff")
			this.navbarType = val
		},
	...
	created() {
		const color = this.navbarColor == "#fff" && this.isThemeDark ? "#191919" : this.navbarColor
		this.updateNavbarColor(color)
		...
		// TEMA CONFIG
		if (this.$store.state.AppActiveUsuario.temaPadrao == "light") {
