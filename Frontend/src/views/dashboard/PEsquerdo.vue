		...
		<div class="vx-row">
			...
			<div class="vx-col w-1/6 mb-1" v-for="item in filterDados" :key="item.centro_destino">
				...
				<vx-card :id="item.centro_destino + 'CA'" ripple hover class="rounded-lg w-36 text-md sm:mt-0 mt-0">
					...
					<small v-show=" item.ultimo_produto == '' " class="text-left font-bold" >{{ 0 }}</small>
			...
			<div class="vx-col w-1/6 mb-1" v-show="rejeitos !== []">
				...
				<vx-card id="RCA" class="rounded-lg w-36 text-md sm:mt-0 mt-0">
					...
					<small class="text-left font-bold" >{{ rejeitos[0].ultimo_produto }}</small>
...
export default{
    ...
    methods: {
		...
		loadData() {
			axios.get(`${backendUrl}/dashesquerdo/`)
		...
		colors() {
			for (var i in this.dados) {
				...
				document.getElementById(qtde_lida).style.color = "#FFFF00"
				document.getElementById('RCA').style.background = ""

				if (parseInt(document.getElementById(qtde_lida).textContent) > 0 && parseInt(document.getElementById(qtde_lida).textContent) != parseInt(document.getElementById(qtde_prevista).textContent)){
					...
					document.getElementById(qtde_lida).style.color = "#FF0000"
					document.getElementById(ultimo_produto).style.color = "#191919"
				}
				if (parseInt(document.getElementById(percentual).textContent) > 0 && parseInt(document.getElementById(qtde_lida).textContent) == parseInt(document.getElementById(qtde_prevista).textContent)){
				...
				if (parseInt(document.getElementById(percentual).textContent) == 0 && parseInt(document.getElementById(qtde_lida).textContent) == 0 ){
