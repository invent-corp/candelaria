	...
	<div id="page-user-list">
          ...
          <div class="vx-card p-6">
               ...
               <div class="flex flex-wrap items-center">
                    ...
                    <div class="flex-grow">
                         ...
                         <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                              ...
                              <div class="p-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                   <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ ptl.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : ptl.length }} of {{ ptl.length }}</span>
                    ...
                    <v-select id="filter-text-box" class="shadow-drop rounded-lg sm:mr-4 sm:order-normal w-64 text-sm" placeholder="Selecione o Mapa PTL" v-model="searchQuery" @input="updateSearchQuery" :options=mapasPtls :reduce="nome => nome.nome" label="nome">
                    </v-select>
...
<script>
	...
	export default {
		data() {
			return {
				...
				defaultColDef: {
                    ...
                    headerTitle: ["Mapa PTL", "Descrição Mapa", "Picking", "Picking Código", "Status"],
                    headerVal: ["mapa_ptl", "mapa_descricao", "picking_descricao", "picking", "situacao"],
                    selectedUsers: [],
                    //EXPORT PDF
                    fileNamePDF: "",
                    formatsPDF:"pdf",
		...
		beforeMount() {
			...
			this.columnDefs = [
				...
				field: 'mapa_descricao',
                    ...
                    {
				headerName: 'Picking',
				field: 'picking_descricao',
                    filter: false,			
				},
                    {
				headerName: 'Picking',
				field: 'picking_situacao',
                    filter: false,
                    cellRendererFramework: 'CellRendererStatus'			
				},
                    {
				headerName: 'Picking Código',
				field: 'picking',
                    filter: false,			
				},
				{
		...
		methods: {
			loadData() {
				axios.get(`${backendUrl}/ptl`)
				.then(res => {this.ptl = res.data})

                    axios.get(`${backendUrl}/ptlcbx`)
