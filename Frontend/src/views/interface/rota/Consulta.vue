                    ...
                    <div class="flex-grow">
                         ...
                         <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                              ...
                              <div class="p-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                   <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ rotas.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : rotas.length }} of {{ rotas.length }}</span>
	...
	export default {
		data() {
			return {
				...
				defaultColDef: {
                    ...
                    headerTitle: ["Descrição", "Código", "Código Inicial", "Código Final", "Status"],
                    headerVal: ["descricao", "codigo", "cod_inicial", "cod_final", "situacao"],
                    selectedUsers: [],
                    //EXPORT PDF
                    fileNamePDF: "",
                    formatsPDF:"pdf",
		...
		beforeMount() {
			...
			this.columnDefs = [
				...
				{
				headerName: 'Código Inicial',
				field: 'cod_inicial',
				filter: true,				
				},
                    {
				headerName: 'Código Final',
				field: 'cod_final',
				filter: true,				
				},
				{
          ...
          methods: {
			loadData() {
				axios.get(`${backendUrl}/rota`)
				.then(res => {this.rotas = res.data})
			},
               Autosize() {
                    var allColumnIds = []
               ...
               exportToExcel() {
                    import('@/components/excel/Export2Excel').then(excel => {
                         ...
                         this.fileNameExcel = 'Invent - Rotas'
                         }
