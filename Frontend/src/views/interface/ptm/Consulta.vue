	...
	<div id="page-user-list">
          ...
          <div class="vx-card p-6">
               ...
               <div class="flex flex-wrap items-center">
                    ...
                    <div class="flex-grow">
                         ...
                         <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                              ...
                              <div class="p-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                   <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ ptm.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : ptm.length }} of {{ ptm.length }}</span>
                    ...
                    <v-select id="filter-text-box" class="shadow-drop rounded-lg sm:mr-4 sm:order-normal w-64 text-sm" placeholder="Selecione o Mapa ptm" v-model="searchQuery" @input="updateSearchQuery" :options=mapasptms :reduce="nome => nome.nome" label="nome">
                    </v-select>
	...
	export default {
		data() {
			return {
				...
				defaultColDef: {
                    ...
                    headerTitle: ["Mapa PTM", "Descrição Mapa", "Endereço", "Endereço Código", "Status"],
                    headerVal: ["mapa_ptm", "mapa_descricao", "endereco_ptm_descricao", "endereco_ptm", "situacao"],
                    selectedUsers: [],
                    //EXPORT PDF
                    fileNamePDF: "",
                    formatsPDF:"pdf",
		...
		beforeMount() {
			...
			this.columnDefs = [
				...
				{
				headerName: 'Descrição Mapa',
				field: 'mapa_descricao',
                    filter: false,			
				},
                    {
				headerName: 'Endereço',
				field: 'endereco_ptm_descricao',
                    filter: false,			
				},
                    {
				headerName: 'Endereço',
				field: 'endereco_ptm_situacao',
                    filter: false,
                    cellRendererFramework: 'CellRendererStatus'			
				},
                    {
				headerName: 'Endereço Código',
				field: 'endereco_ptm',
                    filter: false,			
				},
				{
		...
		methods: {
			loadData() {
				axios.get(`${backendUrl}/ptm`)
				.then(res => {this.ptm = res.data})

                    axios.get(`${backendUrl}/ptmcbx`)
