	...
	<div id="page-user-list">
          ...
          <div class="vx-card p-6">
               ...
               <div class="flex flex-wrap items-center">
                    ...
                    <div>
                         ...
                         <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                              ...
                              <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                   <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ palletItens.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : palletItens.length }} of {{ palletItens.length }}</span>
                    ...
                    <vs-input disabled placeholder="Identificador" class="w-32 text-sm font-medium text-center">> 
                    </vs-input>

                    <vs-input disabled icon-pack="feather" icon="icon-rss" class="mr-3 rounded-lg w-64 text-sm sm:mt-0 mt-4" v-model="palletItens[0].numero_identificador"  >> 
                    </vs-input>
	...
	export default {
		data() {
			return {
				...
				defaultColDef: {
                    ...
                    headerTitle: ["Etiqueta", "SKU", "Leitura"],
                    headerVal: ["etiqueta", "sku", "data_leitura"],
                    selectedUsers: [],
                    //EXPORT PDF
                    fileNamePDF: "",
                    formatsPDF:"pdf",
                    ...
                    printParams: {
                         ...
                         PDF_PAGE_NAME: "Revisa | Pallet Itens",
		...
		beforeMount() {
			...
			this.columnDefs = [
				...
				field: 'acoes',
                    pinned: 'right',
				cellRendererFramework: 'CellRendererDelete',
                    pdfExportOptions: {
                         skipColumn: true
                         }
				}
          ...
          methods: {
			loadData() {
                    const id = this.$route.params.id_pallet
				axios.get(`${backendUrl}/opPalletItens/${id}`)
				.then(res => {this.palletItens = res.data
                         for (let i = 0; ;i++) {
                              this.palletItens[i].data_leitura = moment(this.palletItens[i].data_leitura).format('DD/MM/YYYY HH:mm:ss') 
                         }
                    })
			},
               voltar() {
				this.$router.push("/interface/operacao/oprevisapallet/")
			},
               Autosize() {
                    var allColumnIds = []
               ...
               exportToExcel() {
                    import('@/components/excel/Export2Excel').then(excel => {
                         ...
                         this.fileNameExcel = 'Invent - Revisa Pallet Itens'
                         }
               ...
               exportToPDF() {
                    ...
                    if(this.fileNamePDF == '' || null) {
                         this.fileNamePDF = 'Invent - Revisa Pallet Itens'
                    }
