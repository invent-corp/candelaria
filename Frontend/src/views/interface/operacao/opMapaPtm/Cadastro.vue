	...
	<div id="page-user-list">
          ...
          <div class="vx-card p-6">
               ...
               <div class="flex flex-wrap items-center">
                    ...
                    <v-select class="mr-3 rounded-lg w-64 text-sm sm:mt-0 mt-4" placeholder="Selecionar Mapa" v-model="vMapa.id_mapa_ptm" :options=vMapa :reduce="mapa => mapa.id_mapa_ptm" :menu-props="{ top: true, offsetY: true }" label="nome" >> 
               ...
               <ag-grid-vue 
                    ...
                    overlayNoRowsTemplate='Nenhum Mapa Ptm iniciado !'
...
<script>
	...
	import CellRendererStatus from "../opMapaPtm/cell-renderer/CellRendererStatus.vue"
	import CellRendererVerified from "../opMapaPtm/cell-renderer/CellRendererVerified.vue"
	import CellRendererActions from "../opMapaPtm/cell-renderer/CellRendererActions.vue"
	...
	export default {
		...
		methods: {
			loadData() {
				axios.get(`${backendUrl}/opMapaPtm`)
			...
			loadvMapaSorter() {
				axios.get(`${backendUrl}/opMapaPtmAtivo`)
				.then(res => {this.vMapa = res.data })
			},
			loadCount() {
				axios.get(`${backendUrl}/opMapaPtmcount`)
				.then(res => {this.count = res.data
				})
			},
			iniciarMapa() {
				if (this.count.qtde < 1) {
				const id = this.vMapa.id_mapa_ptm
				axios.put(`${backendUrl}/opMapaPtm/${id}`)
					.then(() => {
