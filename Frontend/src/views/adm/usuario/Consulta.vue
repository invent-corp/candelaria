	...
	<div id="page-user-list">
          ...
          <div class="vx-card p-6">
               ...
               <div class="flex flex-wrap items-center">
                    ...
                    <div class="flex-grow">
                         <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                              ...
                              <div class="p-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                   <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ usuarios.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : usuarios.length }} of {{ usuarios.length }}</span>
                    ...
                    <vs-input icon-pack="feather" icon="icon-search" class="sm:mr-4 mr-0 sm:mt-0 mt-4 sm:w-auto w-full sm:order-normal order-3 " v-model="searchQuery" @input="updateSearchQuery" placeholder="Localizar..." />
...
<script>
	...
	export default {
		data() {
			return {
				...
				defaultColDef: {
                    ...
                    headerTitle: ["Nome", "Login", "Email", "Função", "Status"],
                    headerVal: ["nome", "login", "email", "funcao", "situacao"],
                    selectedUsers: [],
                    //EXPORT PDF
                    fileNamePDF: "",
                    formatsPDF:"pdf",
                    ...
                    printParams: {
                         ...
                         PDF_PAGE_NAME: "Consulta | Usuários",
		...
		methods: {
			loadData() {
                    var adm = this.$store.state.AppActiveUsuario.admin
                    if (adm == 'Sim') {
                         axios.get(`${backendUrl}/usuarioa/""`)
                         .then(res => {this.usuarios = res.data}) 
                    } else {
                         axios.get(`${backendUrl}/usuarioa/sim`)
                         .then(res => {this.usuarios = res.data})     
                    }
			},
               Autosize() {
                    ...
                    // ADMIN
                    if (this.$store.state.AppActiveUsuario.admin == "Sim") {
                         this.gridColumnApi.setColumnVisible('admin', true)
                    } else {
                         this.gridColumnApi.setColumnVisible('admin', false)
                    }
               },
               ...
               exportToExcel() {
                    import('@/components/excel/Export2Excel').then(excel => {
                         ...
                         this.fileNameExcel = 'Invent - Usuários'
                         }
               ...
               exportToPDF() {
                    ...
                    if(this.fileNamePDF == '' || null) {
                         this.fileNamePDF = 'Invent - Usuários'
                    }
