	...
	<div id="page-user-list">
          ...
          <div class="vx-card p-6">
               ...
               <div class="flex flex-wrap items-center">
                    ...
                    <div class="">
                         ...
                         <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                              ...
                              <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                   <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ pickingRota.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : pickingRota.length }} of {{ pickingRota.length }}</span>
                    ...
                    <v-select class="mr-3 rounded-lg w-64 text-sm sm:mt-0 mt-4" placeholder="Selecionar Mapa" v-model="mapa.id_mapa_ptl" :options=mapa :reduce="mapa => mapa.id_mapa_ptl" :menu-props="{ top: true, offsetY: true }" label="nome" >> 
...
<script>
	...
	import CellRendererStatus from "./cell-rota/CellRendererStatus.vue"
	import CellRendererVerified from "./cell-rota/CellRendererVerified.vue"
	import CellRendererActions from "./cell-rota/CellRendererActions.vue"
	...
	export default {
		data() {
			return {
				mapa: null,
				pickingRota: [],
                msgGrid: 'Selecione o Mapa para exibir o relatório !',
				gridApi: null,
				searchQuery: "",
				defaultColDef: {
					sortable: true,
				...
				headerTitle: ["Mapa", "Situação", "Picking", "Situação", "Rota", "Situação", "Código", "Cód Inicial", "Cód Final"],
				headerVal: ["mapa", "s_mapa", "picking", "s_rampa", "rota", "s_rota", "codigo", "cod_incial", "cod_final"],
				//EXPORT PDF
				fileNamePDF: "",
				formatsPDF:"pdf",
				...
				printParams: {
					...
					PDF_PAGE_NAME: "Relatório | Picking X Rota",
		...
		beforeMount() {
			...
			this.columnDefs = [
				...
				{
				headerName: 'Cód Inicial',
				field: 'cod_inicial',
				filter: true,
				},
				{
				headerName: 'Cód Final',
				field: 'cod_final',
				filter: true,
				},
			]
		...
		methods: {
			loadData() {
				axios.get(`${backendUrl}/mapaptl`)
				.then(res => {this.mapa = res.data})
			},
			localizar() {
				const id = this.mapa.id_mapa_ptl
				axios.get(`${backendUrl}/pickingrota/${id}`)
					.then(res => {this.pickingRota = res.data})
			},
			onGridReady() {
				if (this.pickingRota == "") {
					this.msgGrid = "Mapa selecionado não possui Rotas cadastradas!"
				}
			},
			Autosize() {
				var allColumnIds = []
			...
			exportToExcel() {
				import('@/components/excel/Export2Excel').then(excel => {
					...
					this.fileNameExcel = 'Invent - Picking Rota'
					}
			...
			exportToPDF() {
				...
				if(this.fileNamePDF == '' || null) {
					this.fileNamePDF = 'Invent - Picking Rota'
				}
