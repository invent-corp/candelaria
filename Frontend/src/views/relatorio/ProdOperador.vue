                                    ...
                                    <div>
                                        <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                                            ...
                                            <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                                <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ prodoperador.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : prodoperador.length }} of {{ prodoperador.length }}</span>
	...
	export default {
		data() {
			return {
                ...
                headerTitle: ["Qtde", "Login", "Nome", "Tipo"],
                headerVal: ["qtde", "login", "nome", "tipo"],
                selectedUsers: [],
                //EXPORT PDF
                fileNamePDF: "",
                formatsPDF:"pdf",
                ...
                printParams: {
                        ...
                        PDF_PAGE_NAME: "Relatório | Produção X Operador",
		...
		methods: {
			loadData() {
				...
				axios.get(`${backendUrl}/prodoperador/${inicial}&${final}`)
				.then(res => {this.prodoperador = res.data})
            ...
            filtro() {
				...
				axios.get(`${backendUrl}/prodoperador/${dataInicial}&${dataFinal}`)
				.then(res => {this.prodoperador = res.data
                if(this.prodoperador != ''){
                        this.barChart.chartOptions.subtitle= {
                        ...
                        for (let i = 0; ;i++) {
                            operador[i] = this.prodoperador[i].nome  
                            producao[i] = this.prodoperador[i].qtde 
                        }
                    } 
                    if(this.prodoperador == '') {
                        this.barChart.chartOptions.subtitle= {
            ...
            exportToExcel() {
                import('@/components/excel/Export2Excel').then(excel => {
                    ...
                    this.fileNameExcel = 'Invent - Prod Operador'
                    }
            ...
            exportToPDF() {
                ...
                if(this.fileNamePDF == '' || null) {
                    this.fileNamePDF = 'Invent - Prod Operador'
                }
