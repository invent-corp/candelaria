                    ...
                    <div class="flex-grow">
                         ...
                         <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                              ...
                              <div class="p-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                   <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ produtoPicking.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : produtoPicking.length }} of {{ produtoPicking.length }}</span>
...
<script>
	...
	export default {
		data() {
			return {
				...
				defaultColDef: {
                    ...
                    headerTitle: ["SKU", "EAN", "Descrição", "Posição"],
                    headerVal: ["sku", "codigo_produto", "descricao", "picking"],
                    selectedUsers: [],
                    //EXPORT PDF
                    fileNamePDF: "",
                    formatsPDF:"pdf",
                    ...
                    printParams: {
                         ...
                         PDF_PAGE_NAME: "Relatório | Produtos X Posição",
		...
		beforeMount() {
          ...
          methods: {
			loadData() {
				axios.get(`${backendUrl}/produtopicking`)
				.then(res => {this.produtoPicking = res.data})
			},
               Autosize() {
                    var allColumnIds = []
               ...
               exportToExcel() {
                    import('@/components/excel/Export2Excel').then(excel => {
                         ...
                         this.fileNameExcel = 'Invent - Produtos x Posição'
                         }
               ...
               exportToPDF() {
                    ...
                    if(this.fileNamePDF == '' || null) {
                         this.fileNamePDF = 'Invent - Produtos X Posição'
                    }
