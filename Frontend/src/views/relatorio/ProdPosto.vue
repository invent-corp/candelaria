                                ...
                                <div>
                                    <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                                        ...
                                        <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                            <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ prodposto.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : prodposto.length }} of {{ prodposto.length }}</span>
	...
	export default {
		data() {
			return {
                ...
                headerTitle: ["Qtde", "Posto", "Nome"],
                headerVal: ["qtde_produto", "id_posto", "descricao"],
                selectedUsers: [],
                //EXPORT PDF
                fileNamePDF: "",
                formatsPDF:"pdf",
                ...
                printParams: {
                        ...
                        PDF_PAGE_NAME: "Relatório | Produção X Posto",
		...
		methods: {
			loadData() {
				...
				axios.get(`${backendUrl}/prodposto/${inicial}&${final}`)
				.then(res => {this.prodposto = res.data})
            ...
            filtro() {
				...
				axios.get(`${backendUrl}/prodposto/${dataInicial}&${dataFinal}`)
				.then(res => {this.prodposto = res.data
                    if(this.prodposto != ''){
                        this.barChart.chartOptions.subtitle= {
                        ...
                        for (let i = 0; ;i++) {
                            posto[i] = this.prodposto[i].id_posto  
                            producao[i] = this.prodposto[i].qtde_produto
                        }
                    } 
                    if(this.prodposto == '') {
                        this.barChart.chartOptions.subtitle= {
            ...
            exportToExcel() {
                import('@/components/excel/Export2Excel').then(excel => {
                    ...
                    this.fileNameExcel = 'Invent - Prod Posto'
                    }
            ...
            exportToPDF() {
                ...
                if(this.fileNamePDF == '' || null) {
                    this.fileNamePDF = 'Invent - Prod Posto'
                }
