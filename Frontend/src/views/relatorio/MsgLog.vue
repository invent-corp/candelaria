	...
	<div id="page-user-list">
          ...
          <div class="vx-card p-6">
               ...
               <div class="flex flex-wrap items-center">
                    ...
                    <div class="">
                         <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                              ...
                              <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                   <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ msglogs.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : msglogs.length }} of {{ msglogs.length }}</span>
                        ...
                        <flat-pickr class="mr-3 h-10 p-3 w-36 border border-solid rounded-lg d-theme-border-grey-light text-sm" :config="configDataInicialDataPicker" v-model="DataInicial" placeholder="Data Inicial" />
                        <flat-pickr class="mr-3 h-10 p-3 w-36 border border-solid rounded-lg d-theme-border-grey-light text-sm" :config="configDataFinalDataPicker" v-model="DataFinal" placeholder="Data Final" />
                        
                    </div>

                    <v-select class="mr-3 rounded-lg w-32 text-sm sm:mt-0 mt-4" v-model="vLOG" placeholder="Exibir LOG" :options="['Sim', 'Não']"> 
                    </v-select>
                    ...
                    <vs-input icon-pack="feather" icon="icon-search" class="sm:mr-3 mr-0 sm:w-48 w-full sm:order-normal order-3 sm:mt-0 mt-4" v-model="searchQuery" @input="updateSearchQuery" placeholder="Localizar..." />
               ...
               <ag-grid-vue ref="agGridTable" style="height: 370px"
	...
	export default {
		data() {
			return {
                    ...
                    headerTitle: ["Data", "Mensagem", "Tipo", "Envio / Recebe" ],
                    headerVal: ["data", "mensagem", "tipo", "envio_recebe"],
                    selectedUsers: [],
                    //EXPORT PDF
                    fileNamePDF: "",
                    formatsPDF:"pdf",
                    ...
                    printParams: {
                         ...
                         PDF_PAGE_NAME: "Relatório | Mensagem Log",
		...
		methods: {
			loadData() {
                    ...
                    var tipo = ''
                    if(this.vLOG == 'Não' || this.vLOG == undefined) {
                         tipo = 'LOG'
                    }
				axios.get(`${backendUrl}/msglog/${inicial}&${final}&${tipo}`)
				.then(res => {this.msglogs = res.data}
                    )
			},
               filtro() {
                    ...
                    axios.get(`${backendUrl}/msglog/${dataInicial}&${dataFinal}&${tipo}`)
                    .then(res => {
                         this.msglogs = res.data
                              for (let i = 0; ;i++) {
                                   this.msglogs[i].data = moment(this.msglogs[i].data).format('DD/MM/YYYY HH:mm:ss.SSS')
                              }
                         }
                    )
			},
               ...
               exportToExcel() {
                    import('@/components/excel/Export2Excel').then(excel => {
                         ...
                         this.fileNameExcel = 'Invent - MsgLog'
                         }
               ...
               exportToPDF() {
                    ...
                    if(this.fileNamePDF == '' || null) {
                         this.fileNamePDF = 'Invent - MsgLog'
                    }
