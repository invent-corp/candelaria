                            ...
                            <div>
                                <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                                    ...
                                    <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                        <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ prodconferencia.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : prodconferencia.length }} of {{ prodconferencia.length }}</span>
	...
	export default {
		data() {
			return {
                ...
                headerTitle: ["Caixa", "Ordem", "Onda", "SKU", "Qtde Sku", "Qtde Separada", "Qtde Falta", "Qtde Lida", "Data Conferência", "Concluida", "Tipo Conferência", "Conferente"],
                headerVal: ["numero_caixa", "ordem", "onda", "sku", "qtde_sku", "qtde_separada", "qtde_falta", "qtde_lida", "data_conferencia", "concluido", "tipo_conferencia", "nome"],
                selectedUsers: [],
                //EXPORT PDF
                fileNamePDF: "",
                formatsPDF:"pdf",
                ...
                printParams: {
                        ...
                        PDF_PAGE_NAME: "Relatório | Produção X Conferência",
		...
		beforeMount() {
			...
			this.columnDefs = [
                ...
                {
				headerName: 'Qtde Falta',
				field: 'qtde_falta',
				filter: true,			
				},
                {
				headerName: 'Qtde Lida',
				field: 'qtde_lida',
				filter: true,			
				},
                {
				headerName: 'Data Conferência',
				field: 'data_conferencia',
				filter: true,			
				},
                {
				headerName: 'Concluida',
				field: 'concluido',
				filter: true,			
				},
                {
				headerName: 'Tipo Conferência',
				field: 'tipo_conferencia',
				filter: true,			
				},
                {
				headerName: 'Conferente',
				field: 'nome',
				filter: true,			
				}
			]
		...
		methods: {
			loadData() {
				axios.get(`${backendUrl}/prodconferenciacbx`)
				.then(res => {this.ondas = res.data})
//                var inicial = '1800-01-01 00:00'
  //              var final = '1800-01-01 00:00'
	//			axios.get(`${backendUrl}/prodconferencia/${inicial}&${final}`)
	//			.then(res => {this.prodconferencia = res.data})
			},
            filtro() {
				var onda = this.ondas.onda
				axios.get(`${backendUrl}/prodconferencia/${onda}`)
				.then(res => {this.prodconferencia = res.data
                    for (let i = 0; ;i++) {
                        if (this.prodconferencia[i].data_conferencia) {
                            this.prodconferencia[i].data_conferencia = moment(this.prodconferencia[i].data_conferencia).format('DD/MM/YYYY HH:mm:ss')
                        }
                    }
                })
			},
            ...
            exportToExcel() {
                import('@/components/excel/Export2Excel').then(excel => {
                    ...
                    this.fileNameExcel = 'Invent - Prod Conferência'
                    }
            ...
            exportToPDF() {
                ...
                if(this.fileNamePDF == '' || null) {
                    this.fileNamePDF = 'Invent - Prod Conferência'
                }
