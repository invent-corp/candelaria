	...
	<div id="page-user-list">
          ...
          <div class="vx-card p-6">
               ...
               <div class="flex flex-wrap items-center">
                    ...
                    <div class="">
                         ...
                         <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                              ...
                              <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                   <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ rampaRota.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : rampaRota.length }} of {{ rampaRota.length }}</span>
                    ...
                    <v-select class="mr-3 rounded-lg w-64 text-sm sm:mt-0 mt-4" placeholder="Selecionar Mapa" v-model="mapa.id_mapa_sorter" :options=mapa :reduce="mapa => mapa.id_mapa_sorter" :menu-props="{ top: true, offsetY: true }" label="nome" >> 
	...
	export default {
		data() {
			return {
				...
				headerTitle: ["Mapa", "Situação", "Rampa", "Situação", "Rota", "Situação", "Código", "Cód Inicial", "Cód Final"],
				headerVal: ["mapa", "s_mapa", "rampa", "s_rampa", "rota", "s_rota", "codigo", "cod_incial", "cod_final"],
				//EXPORT PDF
				fileNamePDF: "",
				formatsPDF:"pdf",
				...
				printParams: {
					...
					PDF_PAGE_NAME: "Relatório | Rampa X Rota",
		...
		methods: {
			loadData() {
				axios.get(`${backendUrl}/mapasorter`)
				.then(res => {this.mapa = res.data})
			},
			localizar() {
				const id = this.mapa.id_mapa_sorter
				axios.get(`${backendUrl}/ramparota/${id}`)
					.then(res => {this.rampaRota = res.data})
			},
			onGridReady() {
				if (this.rampaRota == "") {
			...
			exportToExcel() {
				import('@/components/excel/Export2Excel').then(excel => {
					...
					this.fileNameExcel = 'Invent - Rampa Rota'
					}
			...
			exportToPDF() {
				...
				if(this.fileNamePDF == '' || null) {
					this.fileNamePDF = 'Invent - Rampa Rota'
				}
