                            ...
                            <div>
                                <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                                    ...
                                    <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                        <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ prodpickingproduto.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : prodpickingproduto.length }} of {{ prodpickingproduto.length }}</span>
	...
	export default {
		data() {
			return {
                ...
                headerTitle: ["Onda", "Ordem", "Caixa", "Data Picking", "SKU", "Produto", "Qtde Sku", "Qtde Separada", "Qtde Short", "Destino", "Posto", "Descrição Posto"],
                headerVal: ["onda", "ordem", "numero_caixa", "data_picking", "sku", "produto", "qtde_sku", "qtde_separada", "qtde_falta", "descricao_destino", "id_posto", "descricao_posto"],
                selectedUsers: [],
                //EXPORT PDF
                fileNamePDF: "",
                formatsPDF:"pdf",
                ...
                printParams: {
                        ...
                        PDF_PAGE_NAME: "Relatório | Produção X Picking Produto",
		...
		beforeMount() {
			...
			this.columnDefs = [
                ...
                {
				headerName: 'Descrição Posto',
				field: 'descricao_posto',
				filter: true,			
				}
			]
		},
		...
		methods: {
			loadData() {
				...
				axios.get(`${backendUrl}/prodpickingproduto/${inicial}&${final}`)
				.then(res => {this.prodpickingproduto = res.data})
			},
            filtro() {
                var dataInicial = this.DataInicial
                var dataFinal = this.DataFinal
				axios.get(`${backendUrl}/prodpickingproduto/${dataInicial}&${dataFinal}`)
				.then(res => {this.prodpickingproduto = res.data
                    for (let i = 0; ;i++) {
                        if (this.prodpickingproduto[i].data_picking) {
                            this.prodpickingproduto[i].data_picking = moment(this.prodpickingproduto[i].data_picking).format('DD/MM/YYYY HH:mm:ss')
                        }
            ...
            exportToExcel() {
                import('@/components/excel/Export2Excel').then(excel => {
                    ...
                    this.fileNameExcel = 'Invent - Prod Picking Produto'
                    }
            ...
            exportToPDF() {
                ...
                if(this.fileNamePDF == '' || null) {
                    this.fileNamePDF = 'Invent - Prod Picking Produto'
                }
