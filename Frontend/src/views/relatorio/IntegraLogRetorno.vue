                    ...
                    <div>
                        <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                            ...
                            <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ integraretornologs.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : integraretornologs.length }} of {{ integraretornologs.length }}</span>
	...
	export default {
		data() {
			return {
                    ...
                    headerTitle: ["Caixa", "Tipo Integração", "Ordem", "Integração Picking", "Integração Fechamento", "Status", "Mensagem Retorno"],
                    headerVal: ["numero_caixa", "tipo_integracao", "ordem", "data_integracao_picking", "data_integracao_close_lpn", "status_retorno", "mensagem_retorno"],
                    selectedUsers: [],
                    //EXPORT PDF
                    fileNamePDF: "",
                    formatsPDF:"pdf",
		...
		beforeMount() {
			...
			this.columnDefs = [
				...
				filter: true,				
                    ...
                    {
				headerName: 'Integração Picking',
				field: 'data_integracao_picking',
				filter: true,				
				},
				{
                    headerName: 'Integração Fechamento',
				field: 'data_integracao_close_lpn',
				filter: true,
				},
                    {
                    headerName: 'Status',
				field: 'status_retorno',
				filter: true,			
				},
                    {
                    headerName: 'Mensagem Retorno',
		...
		methods: {
			loadData() {
				...
				axios.get(`${backendUrl}/integralogretorno/${inicial}&${final}`)
                    .then(res => {this.integraretornologs = res.data}
                    )
			},
               filtro() {
                    var dataInicial = this.DataInicial
                    var dataFinal = this.DataFinal
				axios.get(`${backendUrl}/integralogretorno/${dataInicial}&${dataFinal}`)
                    .then(res => {                         
                         this.integraretornologs = res.data
                              for (let i = 0; ;i++) {
                                   if (this.integraretornologs[i].data_integracao_picking) {
                                        this.integraretornologs[i].data_integracao_picking = moment(this.integraretornologs[i].data_integracao_picking).format('DD/MM/YYYY HH:mm:ss.SSS')
                                   }
                                   if (this.integraretornologs[i].data_integracao_close_lpn) {
                                        this.integraretornologs[i].data_integracao_close_lpn = moment(this.integraretornologs[i].data_integracao_close_lpn).format('DD/MM/YYYY HH:mm:ss.SSS')
                                   }     
             //                      this.integraretornologs[i].onda = this.integraretornologs[i].onda.toString()
      //                             this.integralogs[i].peso_bruto = this.integralogs[i].peso_bruto.toString() + ' Kg'
               ...
               exportToExcel() {
                    import('@/components/excel/Export2Excel').then(excel => {
                         ...
                         this.fileNameExcel = 'Invent - IntegraLog Retorno'
                         }
               ...
               exportToPDF() {
                    ...
                    if(this.fileNamePDF == '' || null) {
                         this.fileNamePDF = 'Invent - IntegraLog Retorno'
                    }
