	...
	<div id="page-user-list">
          ...
          <div class="vx-card p-6">
               ...
               <vs-prompt title="Exportar PDF" class="export-options" @cancel="clearFields" @accept="exportToPDF" accept-text="Exportar" cancel-text="Cancelar" @close="clearFields" :active.sync="activePromptPDF">
                    ...
                    <vs-switch disabled="true" v-model="cellAutoWidthPDF"></vs-switch>
                    </div>
                    <div class="flex p-2">
                    ...
                    <div>
                        <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                            ...
                            <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ integrapallets.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : integrapallets.length }} of {{ integrapallets.length }}</span>
	...
	export default {
		data() {
			return {
                ...
                headerTitle: ["Ident. Pallet", "Caixa", "Data Integração", "Tipo Integração", "Status", "Mensagem Retorno"],
                headerVal: ["numero_identificador", "etiqueta", "data_integracao", "tipo_integracao", "status_retorno", "mensagem_retorno"],
                selectedUsers: [],
                //EXPORT PDF
                fileNamePDF: "",
                formatsPDF:"pdf",
                ...
                printParams: {
                        ...
                        PDF_PAGE_NAME: "Relatório | Integração Pallet Log",
		...
		methods: {
			loadData() {
				...
				axios.get(`${backendUrl}/integrapallet/${inicial}&${final}`)
				.then(res => {this.integrapallets = res.data}
                )
			},
            filtro() {
                var dataInicial = this.DataInicial
                var dataFinal = this.DataFinal
				axios.get(`${backendUrl}/integrapallet/${dataInicial}&${dataFinal}`)
				.then(res => {
                    this.integrapallets = res.data
                        for (let i = 0; ;i++) {
                            if (this.integrapallets[i].data_integracao) {
                                this.integrapallets[i].data_integracao = moment(this.integrapallets[i].data_integracao).format('DD/MM/YYYY HH:mm:ss:SSS')
                            }
                        }
                    }
                )
            ...
            exportToExcel() {
                import('@/components/excel/Export2Excel').then(excel => {
                    ...
                    this.fileNameExcel = 'Invent - Integra Pallet Log'
                    }
            ...
            exportToPDF() {
                ...
                if(this.fileNamePDF == '' || null) {
                    this.fileNamePDF = 'Invent - Integra Pallet Log'
                }
