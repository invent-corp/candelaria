			...
			<div slot="no-body" class="tabs-container px-4 pt-4">
                ...
                <div class="vx-card">
                    ...
                    <div class="flex flex-wrap items-center">
                            ...
                            <div>
                                <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                                    ...
                                    <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                        <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ prodvolumeonda.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : prodvolumeonda.length }} of {{ prodvolumeonda.length }}</span>
                            ...
                            <v-select class="mr-3 rounded-lg w-96 text-sm sm:mt-0 mt-4" placeholder="Selecionar Onda" v-model="ondas.onda" :options=ondas :reduce="ondas => ondas.onda" :menu-props="{ top: true, offsetY: true }" label="onda" >> 
                    ...
                    <ag-grid-vue style="height: 370px"
                            ...
                            :rowData="prodvolumeonda"
                            :animateRows="true"
                            :pagination="true"
                            :suppressPaginationPanel="true"
                            overlayNoRowsTemplate='Selecione a Onda'
	...
	export default {
		data() {
			return {
                ...
                headerTitle: ["Ordem", "Destino", "Volume", "Total Volume", "Caixa", "Data OrderStart", "Data Finalização Picking", "Data Finalização Conferência", "Onda"],
                headerVal: ["ordem", "descricao_destino", "volume", "total_volume", "numero_caixa", "data_orderstart", "data_finalizacao_picking", "data_finalizacao_conferencia", "onda"],
                selectedUsers: [],
                //EXPORT PDF
                fileNamePDF: "",
                formatsPDF:"pdf",
                ...
                printParams: {
                        ...
                        PDF_PAGE_NAME: "Relatório | Volume X Onda",
		...
		beforeMount() {
			...
			this.columnDefs = [
                ...
                {
				headerName: 'Data OrderStart',
				field: 'data_orderstart',
				filter: true,			
				},
                {
				headerName: 'Data Finalização Picking',
				field: 'data_finalizacao_picking',
				filter: true,			
				},
                {
				headerName: 'Data Finalização Conferência',
				field: 'data_finalizacao_conferencia',
				filter: true,			
				},
                {
				headerName: 'Onda',
		...
		methods: {
			loadData() {
				axios.get(`${backendUrl}/prodvolumeondacbx`)
				.then(res => {this.ondas = res.data})
			},
            filtro() {
				var onda = this.ondas.onda
				axios.get(`${backendUrl}/prodvolumeonda/${onda}`)
				.then(res => {this.prodvolumeonda = res.data
                    for (let i = 0; ;i++) {
                        if (this.prodvolumeonda[i].data_orderstart) {
                        this.prodvolumeonda[i].data_orderstart = moment(this.prodvolumeonda[i].data_orderstart).format('DD/MM/YYYY HH:mm:ss')
                        }
                        if (this.prodvolumeonda[i].data_finalizacao_picking) {
                            this.prodvolumeonda[i].data_finalizacao_picking = moment(this.prodvolumeonda[i].data_finalizacao_picking).format('DD/MM/YYYY HH:mm:ss')
                        }
                        if (this.prodvolumeonda[i].data_finalizacao_conferencia) {
                            this.prodvolumeonda[i].data_finalizacao_conferencia = moment(this.prodvolumeonda[i].data_finalizacao_conferencia).format('DD/MM/YYYY HH:mm:ss')
                        }
                    }
                })
			},
            ...
            exportToExcel() {
                import('@/components/excel/Export2Excel').then(excel => {
                    ...
                    this.fileNameExcel = 'Invent - Volume Onda'
                    }
            ...
            exportToPDF() {
                ...
                if(this.fileNamePDF == '' || null) {
                    this.fileNamePDF = 'Invent - Volume Onda'
                }
