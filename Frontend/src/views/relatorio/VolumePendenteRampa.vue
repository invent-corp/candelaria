                    ...
                    <vs-tab class="pt-8" label="Relatório" icon-pack="feather" icon="icon-grid">
                        ...
                        <div class="vx-card">
                                    ...
                                    <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                                        ...
                                        <div
                                            ...
                                            <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize
                                                - 1) }} - {{ volumespendentes.length - currentPage * paginationPageSize
                                                    > 0 ?
                                                currentPage * paginationPageSize : volumespendentes.length }} of {{
                                                volumespendentes.length }}</span>
                            ...
                            <ag-grid-vue style="height: 370px" ref="agGridTable"
                                ...
                                :rowData="volumespendentes" :animateRows="true" :pagination="true"
...
export default {
    data() {
        return {
            ...
            headerTitle: ["Etiqueta", "Rampa", "Destino", "1ª Leitura Invent", "Descrição"],
            headerVal: ["etiqueta", "rampa", "nome_rampa", "primeira_leitura", "descricao"],
            selectedUsers: [],
            //EXPORT PDF
            fileNamePDF: "",
            formatsPDF: "pdf",
            ...
            printParams: {
                ...
                PDF_PAGE_NAME: "Relatório | Volumes Pendentes por Rampa",
    ...
    beforeMount() {
        ...
        this.columnDefs = [
            ...
            {
                headerName: '1ª Leitura Invent',
                field: 'primeira_leitura',
                filter: true,
                width: 180,
            },
            {
    ...
    methods: {
        loadData() {
            ...
            axios.get(`${backendUrl}/volumependenterampa/${inicial}&${final}`)
                .then(res => { this.volumespendentes = res.data })
        },
        filtro() {
            var dataInicial = this.DataInicial
            var dataFinal = this.DataFinal
            axios.get(`${backendUrl}/volumependenterampa/${dataInicial}&${dataFinal}`)
                .then(res => {
                    this.volumespendentes = res.data
                    if (this.volumespendentes.length === 0) {
                        this.$vs.notify({
                            title: 'Aviso',
                            text: 'Nenhum volume pendente encontrado para o período selecionado',
                            color: 'warning',
                            iconPack: 'feather',
                        ...
                        this.$vs.notify({
                            title: 'Sucesso',
                            text: `${this.volumespendentes.length} volumes pendentes encontrados`,
                            color: 'success',
                            iconPack: 'feather',
                            icon: 'icon-check'
                        })
                ...
                .catch(err => {
                    // Use a variável 'err' aqui
                    console.error('Erro ao buscar volumes pendentes:', err);

                    this.$vs.notify({
                        title: 'Erro',
                        text: 'Erro ao buscar volumes pendentes',
                        // ...
                    });
                })
        },
        ...
        exportToExcel() {
            import('@/components/excel/Export2Excel').then(excel => {
                ...
                if (this.fileNameExcel == '' || null) {
                    this.fileNameExcel = 'Invent - Volumes Pendentes por Rampa'
                }
        ...
        exportToPDF() {
            ...
            if (this.fileNamePDF == '' || null) {
                this.fileNamePDF = 'Invent - Volumes Pendentes por Rampa'
            }
