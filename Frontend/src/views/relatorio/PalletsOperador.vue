                                ...
                                <div>
                                    <vs-dropdown vs-trigger-click class="cursor-pointer text-sm">
                                        ...
                                        <div class="p-3 mr-3 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
                                            <span class="mr-2">{{ currentPage * paginationPageSize - (paginationPageSize - 1) }} - {{ palletsoperador.length - currentPage * paginationPageSize > 0 ? currentPage * paginationPageSize : palletsoperador.length }} of {{ palletsoperador.length }}</span>
	...
	export default {
		data() {
			return {
                ...
                headerTitle: ["Qtde", "Login", "Nome"],
                headerVal: ["qtde", "login", "nome"],
                selectedUsers: [],
                //EXPORT PDF
                fileNamePDF: "",
                formatsPDF:"pdf",
                ...
                printParams: {
                        ...
                        PDF_PAGE_NAME: "Relatório | Pallets X Operador",
		...
		methods: {
			loadData() {
                ...
                var inicial = '1800-01-01 00:00:00'
                var final = '1800-01-01 00:00'
				axios.get(`${backendUrl}/palletsoperador/${inicial}&${final}`)
				.then(res => {this.palletsoperador = res.data})
            ...
            filtro() {
				...
				axios.get(`${backendUrl}/palletsoperador/${dataInicial}&${dataFinal}`)
				.then(res => {this.palletsoperador = res.data
                    if(this.palletsoperador != ''){
                        this.barChart.chartOptions.subtitle= {
                        ...
                        for (let i = 0; ;i++) {
                            operador[i] = this.palletsoperador[i].nome  
                            pallets[i] = this.palletsoperador[i].qtde 
                        }
                    } 
                    if(this.palletsoperador == '') {
                        this.barChart.chartOptions.subtitle= {
            ...
            exportToExcel() {
                import('@/components/excel/Export2Excel').then(excel => {
                    ...
                    this.fileNameExcel = 'Invent - Pallets Operador'
                    }
            ...
            exportToPDF() {
                ...
                if(this.fileNamePDF == '' || null) {
                    this.fileNamePDF = 'Invent - Pallets Operador'
                }
